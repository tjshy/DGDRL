...
def evaluate_models_in_directory(dir_path=None, gpu_id=-1, cfg=None):
    ...
    actor = ActorPPO(mid_dim=cfg.act_mid_dim
    ...
    for model_name in model_names:
        ...
        plot(str(model_names.index(model_name)),everyday_assets)
...
def plot(name, amount):
    ...
    plt.ylabel("return")
    plt.legend()
    plt.savefig(result_path+"act/" + name + "return.png")
    plt.close()
...
if __name__ == '__main__':
    ...
    run(cfg)
    gpu_id = cfg.gpu_id
    dir_path=result_path+"StockTradingEnv-v2_PPO_"+str(gpu_id)
