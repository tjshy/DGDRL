from net import ActorPPO, CriticPPO
import torch
...
class AgentPPO:
    def __init__(self
        ...
        self.act_class = ActorPPO
        self.cri_class = CriticPPO
    ...
    def update_net(self, buffer):
        ...
        for train_step in tqdm(range(update_times)):
            ...
            r_sum_cpu=r_sum.detach().cpu()
            r_sum_cpu_df= pd.DataFrame(r_sum_cpu.numpy())
            r_sum_sharp=torch.zeros(len(state))
            for k in range(8,len(state)):
                r_sum_k=r_sum_cpu_df[:k+1]
                r_sum_std=np.std(r_sum_k/r_sum_k.shift(1)-1)
                r_sum_sharp[k]=(r_sum_k/1000000/self.action_dim+1).cumprod().iloc[-1][0]/r_sum_std[0]
            r_sum_sharp=r_sum_sharp.to(self.device)
            obj_critic = self.criterion(value, r_sum_sharp)
