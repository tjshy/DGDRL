...
class GAT_MultiHeads(nn.Module):
    ...
    def forward(self, inputs, Dgraph=None, Sgraph=SGRAPH, requires_weight=True):
...
class FeatureHead(nn.Module):
    ...
    def forward(self, x, x_dg=None):
        scores = self.lstm(x)
        scores, D_attn_weights, S_attn_weights = self.gat(scores,Dgraph=x_dg)
        return scores, D_attn_weights, S_attn_weights
...
class ActorPPO(nn.Module):
    ...
    def forward(self, state, drelation=None):
        action = self.net(self.head(state, drelation)[0]).transpose(1,0).softmax(dim=1)
        return action
    
    def get_attention_weight(self,state,drelation):
        _,D_attn_weights, S_attn_weights=self.head(state, drelation)
        return D_attn_weights,S_attn_weights

    def get_action(self, state, drelation, soft_noise = 0.5):
        a_avg = self.net(self.head(state, drelation)[0]).transpose(1,0).softmax(dim=1)
        a_std = self.a_std_log.exp()
        noise = torch.randn_like(a_avg) * soft_noise
        action = (a_avg + noise * a_std).softmax(dim=1)
        prob = torch.multinomial(action, int(action.shape[1]/10))
        action=torch.zeros_like(action)
        action[0, prob] = 1/int(action.shape[1]/10)
        return action, noise
...
class CriticPPO(nn.Module):
    def __init__(self, mid_dim, mid_layer_num, state_dim, action_dim):
        ...
        self.head = FeatureHead()
    def forward(self, state, drelation=None):
        score = self.net(self.head(state, drelation)[0]).reshape(1,-1) 
